...
- [ ] `cost_sensitivity` for portfolios — the book's net return is linear in cost the same way a single leg's is (gross and turnover both weight-summed), so the same bisection works, but it needs its own derivation against `combine` rather than an assumption that it does
