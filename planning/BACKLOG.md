...
- [x] Ledoit-Wolf test for strategy-vs-benchmark Sharpe difference — the "does it beat buy-and-hold" question with a p-value; `sharpe_test.py`, paired HAC + studentised bootstrap, two-sided, on every `/api/backtest` and `/api/portfolio` as `benchmark_test`; 32 tests, 7 of 8 mutations caught and the survivor documented; size and power measured (DECISIONS 23 Sep) — on the results card since PR #49
- [x] Show `benchmark_test` on the results card — `BenchmarkTest.jsx`, a strip under the headline metrics on `/dashboard` and `/demo`, showing both Sharpes, the gap ± its standard error and the p-value, coloured only when the test actually calls it (PR #49, 23 Sep)
